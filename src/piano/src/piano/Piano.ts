...
export class Piano extends ToneAudioNode<PianoOptions> {
	...
	keyDown({ note, midi, time = this.immediate(), velocity = 0.8 }: KeyEvent): this {
		...
		console.debug(`piano keydown note `, note)
		console.debug(`piano keydown midi `, midi)

		if (this.loaded && this.maxPolyphony > this._heldNotes.size + this._sustainedNotes.size) {

			time = this.toSeconds(time)

			console.debug(`note is string? ${isString(note)}`)
			if (isString(note)) {
				let midiObj = Midi(note)
				console.debug(`midiObj `, midiObj)

				midi = Math.round(midiObj!.toMidi())
				console.debug(`to midi `, midiObj!.toMidi())
			}
